			...
			for (var i = 0; i < tweets.length; i++) {
				log(
					chalk.bold.underline.bgMagentaBright(
						'Follow me on twitter @Myles_Code'
					)
				);
...
function spotifyThisSong() {
	...
	spotify.search({ type: 'track', query: value }, function(err, data) {
		...
		log(
			...
			chalk.bold.cyan('\nPREVIEW LINK: '),
			chalk.red.underline(data.tracks.items[0].href),
			chalk.bold.magentaBright('\nALBUM: '),
...
function movieThis() {
	...
	request(queryUrl, function(error, response, body) {
		...
		if (!error && response.statusCode === 200) {
			...
			log(
				chalk.bold.cyan('\nTITLE: '),
				chalk.bold.whiteBright(JSON.parse(body).Title),
				chalk.bold.magentaBright('\nRELEASED IN: ') +
					chalk.bold.whiteBright(JSON.parse(body).Year),
				chalk.bold.cyan('\nIMDB RATING: '),
				chalk.bold.redBright(JSON.parse(body).Ratings[0].Value),
				chalk.bold.magentaBright('\nROTTEN TOMATOES RATING: '),
				chalk.bold.redBright(JSON.parse(body).Ratings[1].Value),
				chalk.bold.cyan('\nCOUNTRY RELEASED IN: '),
				chalk.bold.whiteBright(JSON.parse(body).Country),
				chalk.bold.magentaBright('\nLANGUAGE: '),
				chalk.bold.whiteBright(JSON.parse(body).Language),
				chalk.bold.cyan('\nPLOT: '),
				'"' + chalk.bold.bgMagenta(JSON.parse(body).Plot) + '"',
				chalk.bold.magentaBright('\nACTORS: '),
				chalk.bold.green(JSON.parse(body).Actors)
			);
		}
	});
}
